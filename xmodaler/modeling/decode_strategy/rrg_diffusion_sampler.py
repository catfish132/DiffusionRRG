...
from xmodaler.functional import pad_tensor, bits_to_decimal, beta_linear_log_snr, alpha_cosine_log_snr, \
    log_snr_to_alpha_sigma, right_pad_dims_to, log
...
class RRGDiffusionSampler(DecodeStrategy):
    ...
    def _forward(self, batched_inputs, model):
        ...
        if kfg.ATT_MASKS in batched_inputs:
            pass
        else:  # 做图片的视觉编码然后生成mask
            att_feat = model.reshape_feat(model.visual_backbone(batched_inputs[kfg.IMAGES]))
            if kfg.IMAGES2 in batched_inputs:
                att_feat2 = model.reshape_feat(model.visual_backbone(batched_inputs[kfg.IMAGES2]))
